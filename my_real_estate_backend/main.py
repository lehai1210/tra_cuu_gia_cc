...
# SỬA ĐỔI Ở ĐÂY: PropertyNumberItem sẽ chứa cả property_code và property_number (để hiển thị)
class PropertyNumberItem(BaseModel):
    property_code: str  # Dùng làm value cho dropdown, gửi lên API /lookup
    property_number: str # Dùng để hiển thị cho người dùng
...
async def lookup_property_price(
    ...
    # API /lookup vẫn nhận property_number (là property_code) để tra cứu
    ...
    try:
        ...
        try:
            project_lookup_response = supabase_client.from_("dim_projects") \
                ...
                .single() \
                .execute()
            project_id_found = project_lookup_response.data.get("id") if project_lookup_response.data else None
        except PostgrestAPIError as e_project_lookup:
            if hasattr(e_project_lookup, 'code') and e_project_lookup.code == "PGRST116":
                raise HTTPException(status_code=404, detail=f"Không tìm thấy dự án với tên: '{project_name_input}'")
            else:
                print(f"Lỗi API Supabase khi tìm project_id: {e_project_lookup}")
                raise HTTPException(status_code=getattr(e_project_lookup, 'status', 500), detail=str(e_project_lookup))
        ...
        if property_number: # property_number ở đây là property_code được gửi từ frontend
            query = base_query.eq("property_code", property_number) 
            try:
                response_prop = query.single().execute()
                property_data_item = response_prop.data
            except PostgrestAPIError as e_prop_single:
                if hasattr(e_prop_single, 'code') and e_prop_single.code == "PGRST116":
                    raise HTTPException(status_code=404, detail=f"Không tìm thấy căn hộ với mã '{property_number}' tại dự án '{project_name_input}' và tỉnh/thành '{region_name}'.")
                else:
                    print(f"Lỗi API Supabase khi tìm tài sản cụ thể: {e_prop_single}")
                    raise HTTPException(status_code=getattr(e_prop_single, 'status', 500), detail=str(e_prop_single))
        else: 
            query = base_query.order("property_code").limit(1)
            response_prop = query.execute()
            error_obj_prop_list = getattr(response_prop, 'error', None) 
            if error_obj_prop_list:
                raise HTTPException(status_code=getattr(error_obj_prop_list, 'status', 400), detail=getattr(error_obj_prop_list, 'message', str(error_obj_prop_list)))
        ...
        # Sử dụng property_number (tên thân thiện) để hiển thị nếu có, nếu không thì dùng property_code
...
# SỬA ĐỔI Ở ĐÂY: Endpoint /property-numbers-list
...
async def get_property_numbers_list(
    ...
    """
    Lấy danh sách các mã căn hộ (property_code) và tên hiển thị (property_number)
    thuộc một dự án (theo ID) và vùng cụ thể.
    Đảm bảo bảng 'properties' có cột 'property_code' và 'property_number'.
    """
    try:
        ...
        response = query.order("property_number").execute() # Sắp xếp theo tên hiển thị
        error_obj = getattr(response, 'error', None) 
        if error_obj:
            status_code = getattr(error_obj, 'status', 400)
            message = getattr(error_obj, 'message', str(error_obj))
            # Kiểm tra lỗi cụ thể nếu cột 'property_number' không tồn tại
